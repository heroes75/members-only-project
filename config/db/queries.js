...
async function insertNewMessage({title, text}, userId) {
    await pool.query('INSERT INTO messages (title, message, added, authorid) VALUES ($1,$2, NOW(), $3)', [title, text, userId])
}

async function getAllMessages() {
    const {rows} = await pool.query('SELECT users.id AS userId, firstname, lastname, member, admin, messages.id AS messageId, title, message, added, authorid FROM users JOIN messages ON users.id = authorid')
    return rows
}
